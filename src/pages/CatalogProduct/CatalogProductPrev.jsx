    ...
    <div className='p-3'>
        ...
        <div className="info-product mt-2 pb-3">
            <div className="img-product" id="img-product">
                <img className='rounded rounded-3' src={product.image} alt="Product Image" width="100%" style={{objectFit: 'cover'}} onError={(e) => { e.target.onerror = null; e.target.src = 'https://i.imgur.com/2a0RWOy.jpg'; }} />
            </div>
            <div className="title-product mt-3 mb-4" style={{ position: 'relative' }}>
                <div className="name-product" id="name-product">
                    <p className='fs-5 fw-bold mb-0' style={{ color: '#2B2A4C' }}>{product.nama_produk}</p>
                </div>
                <div className='d-flex justify-content-between align-items-center'>
                    <div className="price-product" id="price-product">
                        <p className='fs-5 fw-bold mb-0' style={{ color: '#ea906c' }}><span>Rp </span>{product.harga}</p>
                    </div>
            ...
            <div className="d-grid gap-2 btn-checkout" id="btn-checkout">
                <button className="btn text-white fw-bold" type="button" style={{backgroundColor: '#b31312'}}>
                    <Link to={`/homepage/search/produk/checkout/${product._id}`} className='text-decoration-none text-white'>Checkout</Link>
                </button>
            </div>
        </div>
